...
begin
  if (TryStrToFloat(EditR1.text,a.R1) = false) or (StrToFloat(EditR1.text)<=0) then
  begin
     EditR1.Color:=clRed;
  ...
  if (TryStrToFloat(EditR2.text,a.R2) = false) or (StrToFloat(EditR2.text)<=0) then
  begin
     EditR2.Color:=clRed;
  ...
  if (TryStrToFloat(EditR3.text,a.R3) = false)  or (StrToFloat(EditR3.text)<=0) then
  begin
     EditR3.Color:=clRed;
     ShowMessage('Неправильно введён параметр m1');
     exit;
...
var
   NameFile:string;
begin
  if (SaveDialog.Execute) and (SaveDialog.FileName<>'') then begin
     datafromform();
...
var
   ...
   ftxt:text;
begin
     if not (SaveOtchet.execute) and (SaveOtchet.FileName<>'') then exit;
